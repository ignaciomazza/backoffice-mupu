// src/components/bookings/BookingPermissionsConfig.tsx
"use client";
...
import {
  ...
  normalizeRole,
  type BookingComponentAccessRule,
...
export default function BookingPermissionsConfig() {
  const { token, role } = useAuth();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [users, setUsers] = useState<User[]>([]);
  const [rules, setRules] = useState<BookingComponentAccessRule[]>([]);
  ...
  const updateRule = (userId: number, components: BookingComponentKey[]) => {
    if (!userId) return;
    setRules((prev) => {
      const idx = prev.findIndex((rule) => rule.id_user === userId);
      const updated: BookingComponentAccessRule = {
        id_user: userId,
        components,
      };
      const nextList =
    ...
    (async () => {
      try {
        ...
        const [usersRes, rulesRes] = await Promise.all([
          ...
          authFetch(
            "/api/bookings/permissions?scope=all",
            { cache: "no-store" },
            token,
          ),
        ]);
        ...
        if (rulesRes.ok) {
          ...
          const parsed = normalizeBookingComponentRules(payload?.rules);
          if (alive) setRules(parsed);
        } else if (alive) {
          setRules([]);
        }
      } catch {
        toast.error("No se pudo cargar permisos de reservas.");
      } finally {
  ...
  const saveRules = async () => {
    ...
    try {
      ...
      const nextRules = normalizeBookingComponentRules(body?.rules);
      setRules(nextRules);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold">Acceso a componentes</h2>
          <p className="text-sm opacity-70">
            Define que partes de la reserva puede usar cada usuario.
          </p>
        </div>
      ...
      {selectedRule ? (
        <div className={`${GLASS} space-y-3 p-4`}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <h3 className="text-sm font-semibold">Componentes permitidos</h3>
              <p className="text-xs opacity-70">
                Si no seleccionas ninguno, aplican solo los permisos por rol.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {BOOKING_COMPONENTS.map((component) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={selectedRule.components.includes(component.key)}
                  onChange={() =>
                    updateRule(
                      selectedRule.id_user,
                      toggleKey(selectedRule.components, component.key),
                    )
                  }
                  disabled={saving}
                />
