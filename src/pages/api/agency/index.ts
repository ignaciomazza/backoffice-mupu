...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    const { name, address, phone, email, tax_id, website, foundation_date, logo_url } = req.body;
    ...
    try {
      const newAgency = await prisma.agency.create({
        data: {
          ...
          tax_id,
          website,
          foundation_date: foundation_date ? new Date(foundation_date) : undefined,
          logo_url,
        },
      });
    ...
    try {
      ...
      if (!existingAgency) {
        return res.status(404).json({ error: "No se encontró ninguna agencia para actualizar" });
      }
