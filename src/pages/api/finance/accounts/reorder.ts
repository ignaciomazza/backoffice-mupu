// src/pages/api/finance/accounts/reorder.ts
...
export default async function handler(
  ...
  try {
    ...
    const owned = await prisma.financeAccount.findMany({
      where: { id_agency, id_account: { in: ids } },
    ...
    const results = await prisma.$transaction(
      items.map((it) =>
        prisma.financeAccount.updateMany({
          where: { id_agency, id_account: it.id },
    ...
    console.error("Accounts reorder error:", e);
