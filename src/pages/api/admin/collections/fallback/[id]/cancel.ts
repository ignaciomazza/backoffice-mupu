...
import { cancelFallbackIntent } from "@/services/collections/dunning/service";
...
export default async function handler(
  ...
  try {
    const result = await cancelFallbackIntent({
      fallbackIntentId: id,
      actorUserId: auth.id_user,
      source: "MANUAL_CANCEL_ENDPOINT",
    });
    return res.status(200).json(result);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "No se pudo cancelar fallback";
